...
> | `flow-auth-desktop.html`, `auth.css` | §24 Sign in & onboarding — desktop |
> | `flow-chat-mobile.html`, `chat.css`, `chat-demo.js` | §27 Chat with host — mobile |
> | `flow-chat-desktop.html`, `chat.css`, `chat-demo.js` | §28 Chat with host — desktop |
...
> | `flow-travel-guide-v3.html`, `flow-travel-guide-desktop-v3.html`, `travel-guide-v3.css`, `travel-guide-v3.js` | §35.8 AI Travel Guide (**current** — Redesign 2.0 refresh of the shipped view) |
> | `flow-travel-guide-v2.html`, `flow-travel-guide-desktop-v2.html`, `travel-guide-v2.css`, `travel-guide-v2.js` | §35 AI Travel Guide (previous — UX critique of the old legacy view) |
...
> | `flow-lounge-mobile.html`, `flow-lounge-desktop.html`, `lounge.css`, `lounge-flow.js` (+ lounge cards in `home*.html`, `flow-upselling-*.html`, scenes in `photos.css`) | §40 VIP airport lounge (**proposal — new feature**) |
>
...
> hanging forever, and closing its two curtains over a **shape-matched skeleton that is
> already rendered** instead of over a dead rectangle. The surface stays V1's — every
> change is behavioural. Documents four confirmed defects in the shipped component
...
> for a while was duplicated by a payments-embedded deposit-chooser; that duplicate is
> gone and §33 is the single prototype for the decision.
...
> gate deep-links the blocking task and the empty state points to Vela tips / FAQ. The
> desktop sidebar's existing "Guidebooks" item is now the destination, with the chapter
> index living in the Vela rail.
...
> branded gradient on mobile and a brand-panel split on desktop; the Minimal skin
> (`html.glass-lite`, overrides in `auth.css`) is baked in — no glass switch.
...
· **Prototypes:** `flow-registration.html#r-hub` (mobile), `guest-registration-desktop.html`
(desktop) — the roster lives inside the Guest-registration pages; the standalone
`flow-guests-*.html` reference pages (and before them the V2 `gv-*` proposal) were removed
as duplicates. The roster below is the design; the interactive journey is §2.1–§2.3.

### 2.0.1 The roster row

One row per guest, carrying its own way in — a status (**Complete** / **Incomplete**) paired
with the action that clears it (*Finish registration* for missing details, *Finish verification*
for a pending ID), an overflow menu and a chevron. No avatars. Section labels sit in the card
header (Responsible for booking · Other guests), and the add-a-minor row closes the list.
...
Delete is **two-step and reversible**:
...
3. Confirm → row hides optimistically → **Undo toast** for 10 s, and a `role="status"`
   live region announces both the removal and the undo.

> Documented intent — the interactive implementation lived in the removed V2 frames and is
> not currently prototyped; the static roster shows the kebab affordance only.
...
booking** (crown) → **Other guests** (users) → **Minors — under 18** (child) → docked CTA.

Minors are their **own card**, not an "Add a minor under 18" row tucked under the adults.
They follow a different rule — no ID photos, no identity verification, registered by an
adult on the booking — and burying that in a second add-row made the roster's one real
distinction invisible at exactly the moment a guest is deciding whom to add. The heading
carries the distinction on its own; minors render as ordinary `.guest-row`s inside it.
...
Document choice at `d-choice`* (Vela rail visible) — nationality + document type are the
screen's only decision; the phone handoff is a secondary affordance in the rail. Then
desktop mirrors the full mobile case set:
...
- **Continue on phone** (recommended): QR + live sync `d-qr` → auto `d-qr-done` →
  ...
  simulated sync.
- **Webcam**: `d-choice` is itself the document choice (same `data-set-doc` branch point
  as `s-doc`) → `d-cam` (auto) → `d-cam-captured` → branch: [passport: `d-live-1..3` (auto) →
...
`d-complete` (full-bleed celebration) and `d-cam-denied` (the page already *is* the
enable steps and carries Try again / phone / upload itself). Rail content swaps per
...
|---|---|---|
| `d-qr`/`d-qr-done` | While your phone connects | webcam (`d-cam`), upload (`d-crop`) |
...
| `d-cam-denied` | **no rail** — the page is the instructions plus all three escapes; the rail repeated both |
| `d-unavailable` | Why you can't start yet (no progress module) | share link (`m-share`) |
**Removed:** `d-upload`, the old standalone dropzone view with a multi-file list. It
was kept behind a crossed-out overlay for reference, but the real app has no upload
screen — "Upload a file" opens the native picker directly, one file per side, and the
chosen photo lands on crop & confirm (`CameraCapture.openFileDialog` → `PhotoUpload`).
The reference copy outlived its usefulness; `d-crop` is the design. Its `.dropzone` /
`.upload-row` / `.dep-*` styles went with it.
...
| Glass | — | Minimal (`html.glass-lite`) baked onto `<html>`; `search-v2.css` carries the overrides, `housing-iv-v2.css` adds `.hv-typed` and — since it loads after `iv.css` — the `.status-pill` fallbacks |
...
**Prototypes:** `home.html`, `home-desktop.html` (+ `home.css`, `upsell.css`)
...
`useChecklistSteps.ts` / `useHomeData.ts`. The prototype keeps this component set and its
data model — it re-composes them, it does not restructure the code.

The Home job, in priority order: **(1) what do I do next, (2) what does finishing
unlock, (3) everything else.**

**Hero** (`CheckinHeroStrip`) — check-in/out, nights and location, plus a glass
**arrival countdown** that ties finishing early to the reward ("Check-in opens in 3 days
— finish 2 tasks to get your keys instantly").

**Checklist** (`CheckinChecklist`) — one row per step with a progress ring and a
**"next up" spotlight** on the pending row (left accent bar, soft blue wash, pulsing
badge, "Next up" chip); completed rows carry the outcome fact ("€ 380.00 paid"). Status
reads by text + icon, never colour alone. Row targets mirror §1.4 rows 2–4. The all-done
state celebrates and surfaces the now-unlocked keys.

**Stay tools** (`YourStaySection`) — keys (with a **Locked** state until check-in is
done, deep-linking the "why?" gate §15) and, on desktop, experiences.

**Guidebooks are listed individually** (`GuidebooksNavigationList` → `GuidebooksList`),
not behind a generic "Guidebooks" tile: a named book ("Welcome & House Manual", "Getting
Around Málaga") is a stronger invitation than a category, and the guest's most common
question — how do I get in — is one tap from Home instead of two. `.home-guides` rows
carry an icon, the title, the host's description and a **format tag** covering all three
`content_type`s (§21.1): **Guide** (`EDITOR`), **Web** (`HTML`), **PDF**. Each row deep-
links to the matching reader in §21/§22; "See all" opens the list destination.

Chrome: glass top bar + bottom dock and a left menu drawer (mobile); navy sidebar with a
progress module and per-task badges + host-contact right rail (desktop).
...
**Prototype:** `vela.html` + embedded in **every reservation-context page**.

Desktop: right rail (324 px) — progress module, quick actions (Share booking link → `m-share`,
QR mobile-switch → `m-qr`, chat), contextual tips. Mobile: FAB in the top bar →
...
**Vela is a permanent fixture inside the flows, not a per-flow decision.** A helper that
appears on some task screens and not others reads as broken rather than contextual, so
every flow the guest works through carries it, in exactly one of two shapes: the top-bar
FAB → `m-vela` sheet on mobile, the 324 px rail on desktop. Where a desktop flow already
owns its third column (chat's `ctx-rail`, the travel guide's `tg-rail`), Vela heads
...
the per-task rows. Vela belongs where a guest can get *stuck*, which is inside a step.
Desktop Home keeps its plain **"Need help? → Chat with your host / FAQ & support"** cards
in the rail as the escape hatch.
...
**Placement rule:** within a booking Vela is present on every screen **except** (a) over
...
- Item rows mirror `useBuildDisplayedPayments`: icon + name + human meta; tourist taxes
  ...
  (→ §33 `dp-edit` / `dpd-edit` — the real `onEditProtection` navigates to
  `PATHS.deposits`, so the link leaves payments); PMS-origin items (booking
  stay) have **no kebab** (`isPMSorigin`).
- **Kebab ⋯ semantics mirror `CartItemRow`'s `ResponsiveDropdown`:** every
  non-PMS row has ⋯ with "Add to pay later" — including tourist taxes, which
  can be postponed but never removed; "Remove item" appears only on removable
  upsells (`isRemovable` — not third-party, and pet fees refuse with a toast).
  Mobile renders the menu as a bottom sheet (`m-item-menu` on the breakfast
  basket — pay later + remove; `m-item-menu-tax` on taxes — pay later only,
  with the can't-remove explanation), desktop as a `rowmenu.js` popover
  (`cm-food`, `cm-tax`). Desktop menu items are **text only** — no leading
  glyphs: two or three rows of title + description don't need icons to be
  scannable, and the glyphs read as decoration next to the row's own icon
  chip.
- **One inline action per row.** "What is this?" (`m-taxinfo`, the real
  `TaxesInformationModal`) lives in the ⋯ menu only, not beside
  "Add exemptions". `CartItemRow` used to render both inline in a
  `flex-wrap` span, but at cart width they wrap and the taxes row grows to
  ~1.5× its neighbours' height — a row that looks structurally different from
  the rows around it for no reason the guest can see. Fidelity to that detail
  costs more than it buys, so the real row now keeps only the edit link inline.
  The hold section
...
- **Cart rows use icon chips, not offer photographs — settled.** The data is
  there and it is free: `CartPayment` carries no image, but
  `upselling_deal_id` joins to the deal list `MyCart` already fetches for the
  pet-fee check, so `offer.image_url` would cost no extra request. It was
  prototyped **twice and reverted twice**; this bullet is the record so it
  isn't proposed a third time. The reason is the cart's job: make three kinds
  of money legible (§7.1), not re-sell what the guest already chose. A photo
  box beside icon-chip rows reads as a different component rather than the
  same row with richer content, and it pulls weight toward the two upsell
  lines and away from the ledger — which is the only thing on the screen the
  guest must not misread. Photos stay where the selling happens (§13
  upselling, §34 booking); by the cart, the decision is already made.
- **Removing an item** (`p-removing` → `p-removed`, `m-remove`,
  `m-remove-error`) — the destructive half of the ⋯, mapped to
  `handleRemove` → `useMutateDeals.deleteDealMutation`:
  - A **confirm step is added** (`m-remove` → the real `RemoveItemModal`),
    which the pre-redesign code did not
    have. Not for politeness: `deleteDeal` is permanent, and getting the item
    back means re-booking the offer — which `getAvailableDates()` may refuse
    once the booking window has closed (§13.4 `u-request-nodates`). The sheet
    says both true things: nothing was charged (so no refund, no order-history
    entry), and this is not an undo.
  - `p-removing` is `CartItemRow`'s `isRemoving`: the row **stays in place**,
    dimmed, its menu disabled and its amount replaced by a spinner; the ledger
    and CTA keep the old total. A row that vanishes optimistically is a lie if
    the mutation fails.
  - Success (`onSuccess` refetches deals + cart): the row is gone, the total
    drops, and the toast offers "Book it again" — a link to the offer (§13),
    deliberately **not** labelled Undo, because it is a new booking.
  - Failure (`useMutateDeals.onError` → `displayError`): `m-remove-error`,
    the app's ErrorModal — the item is still in the cart and still unpaid.
    Reachable from the explicit fork on `p-removing`, which offers both real
    outcomes rather than auto-advancing to the happy one.
- **The pet fee is the one removal the app refuses** (`p-petfee` →
  `p-petfee-blocked`). `handleRemove` looks the row up in the deals list and,
  if `offer.category === OFFER_CATEGORIES.petFee`, fires
  `showToast.error('pet_fees_cannot_be_removed')` and returns `false` — the
  mutation never runs. The prototype keeps that honest: `isRemovable` is true
  (it is a normal upselling deal), so **"Remove item" is present in the menu**
  and the refusal only happens after the tap. The redesign adds what the bare
  toast had no room for — a "Why can't I remove this?" link to `m-petfee`
  (the real `PetFeeInfoModal`), which
  explains the fee is the property's and routes to §27 chat, the only place it
  can actually be changed. In the app the link rides the refusal toast as its
  action and then stays on the row, so a guest who dismissed the toast can
  still find the explanation. The pet-fee row appears in this scenario only.
...
### 7.3 Pay (`p-form`, `p-form-addon`, `p-3ds`)
...
- **`p-form-addon` is the other real provider** (`PAYMENT_PROVIDERS.addon` →
  `AddonPaymentForm` + `useAddonPayments`, Global Payments' Realex HPP). Its
  constraint is harder than Stripe's: the **entire form — fields, validation
  and the pay button — is GP's hosted page in an iframe** (`rxp-js` embedded
  mode). No wallet tabs, no Appearance API, no CTA dock of ours; 3-DS runs
  inside the same window, so there is no separate `p-3ds` on this path. Flow
  differences the prototype documents: from the cart, Pay first **creates the
  multi-payment** (brief loading on the cart CTA — `startPaymentProcess` →
  `onDataRequired` navigates with `paymentId`), a failed HPP init returns to
  the cart, and closing the error modal also goes back to the cart. The
  redesign owns everything around the fence: page head, ledger, recovery
  states. Submission inside the frame → `p-processing` (same timeline).
...
- `p-success`: "€380.00 paid" + itemized receipt (items, dashed hold line, card ····4242,
  date) + "Receipt sent to {email}" + download PDF + **"View order history"**
  (→ `p-history`) — the real `onSuccessPaymentForm` navigates to
  `PATHS.payments.ordersHistory`, so where the receipt lives must be one tap
  away. Primary CTA follows the §1.4
...
### 7.5 Order history (`p-history`)

- History mirrors `useBuildDisplayedOrderHistory` / `OrderHistoryGroup` **1:1** —
  it does NOT invent per-movement rows. The backend returns four fixed groups
  (`payment_details` / `pre_authorizations` / `retentions` / `refunds`, each a
  `PaymentOrderHistory` with `date`, `total_amount_with_commission`, `items[]`);
  empty groups are filtered. The page renders **one card per group**:
  - Above the card: `date` (`D MMM YYYY`) left, kind badge right — green
    **Paid** (`payment_details`), dashed **Hold** (`pre_authorizations` /
    `retentions`), blue **Refunded** (`refunds`).
  - Card header row: kind icon (receipt / shield / undo — green chip only for
    paid), the **real group title** ("Payment details", "Pre-authorizations",
    "Refunds"), meta "`N items` · `card ···· {last4}`" (one shared
    `cards_info[0]` for every group — there is no per-order card), and the
    group total (`total_amount_with_commission`; neutral color for holds).
  - Item lines: `name` `· label_text` — `amount_with_commission`. Not
    clickable — the real rows have no handlers and no per-entry receipt.
  - Footer strip: the group's real helper text (`pre_authorization_helper_text`,
    `retention_helper_text`, `refunds_helper_text`) verbatim; `payment_details`
    has none.
  Mock story: Payment details €380.00 (4 items), Pre-authorizations €410.00
  (deposit + early check-in + vineyard), Refunds €12.50 (breakfast basket);
  Retentions renders identically when the backend returns it. Footer note
  explains "pending" statement lines (`holds_appear_as_pending_note`).
- **The protection chooser is not a payments screen.** It lived here as
  `p-deposit`/`pd-deposit` before §33 existed; the standalone `DepositView`
  now owns the decision in every shape the backend can produce, so the
  duplicate was removed rather than kept in sync. "Edit protection" on the
  deposit hold links out to §33's edit state (`dp-edit`/`dpd-edit`), which is
  what `onEditProtection` → `PATHS.deposits` actually does.
...
`p-cart`* · `p-later` · `p-removing` · `p-removed` · `p-petfee` ·
`p-petfee-blocked` · `p-history` · `p-empty` · `p-form` · `p-form-addon` ·
`p-3ds` · `p-processing` · `p-slow` · `p-success` · `p-error` · modals `m-vela`,
`m-hold`, `m-item-menu`, `m-item-menu-tax`, `m-item-menu-pet`, `m-remove`,
`m-remove-error`, `m-petfee`, `m-taxinfo`
(* = `data-start`; autonext: processing→success 3.0 s, slow→success 3.4 s.
`p-removing` deliberately does **not** autonext — it forks to both real
outcomes.)
...
- `pd-cart`: two-column grid — item list left, **sticky ledger** right with the CTA
  ...
  The §7.2 kebab semantics render as `rowmenu.js` popovers (`cm-food` on the
  breakfast basket — pay later → `pd-later` / remove; `cm-tax` on taxes — pay
  later + "What is this?" → `m-taxinfo`); the cart card is `overflow:visible`
  so the popovers aren't clipped.
- `pd-later`: the pay-later state (§7.2) with the desktop grid intact — breakfast
  parked in the "Pay later" box between cart and holds, "Send back to cart" →
  `pd-cart`, ledger and CTA drop to €367.50.
...
- `pd-form-addon`: the §7.3 Addon (Global Payments HPP) variant — the whole form
  incl. the pay button inside the fenced iframe, no wallets, submission →
  `pd-processing`.
- `pd-success` is **full-bleed** (no Vela rail, per §6 placement rule); sidebar flips
  Payments/protection/taxes badges to ✓ and progress to 75%. Like `p-success`
  it carries "View order history" (→ `pd-history`) — the real `onSuccess`
  destination.
...
| `pd-later` | progress step 1 + pay-later tips (send-back works before or after paying; no deadline) | — |
| `pd-removing` | **minimal** — one patience tip (the item stays if the property doesn't confirm) | — |
| `pd-removed` | "Book it again" (→ §13, a fresh booking, not an undo) + nothing-to-refund | — |
| `pd-petfee` / `pd-petfee-blocked` | why the fee is the property's (`m-petfee`) + message the property (→ §28) | — |
...
| `pd-form-addon` | progress step 2 + security tips (Global Payments, in-window 3-DS) | back to cart (back-pill) |
| `pd-processing` | **minimal** — one patience tip, no ask box (liveness precedent) | — |
| `pd-error` | **no rail** — see below | — |

**`pd-error` carries no rail.** The decline screen is already nothing but
the explanation and the way out: the body names the cause ("a card limit or an
online-payments setting — your banking app often says exactly why"), states that nothing
was charged, and offers Try again / different card / back to cart. The rail restated all
three, one tip each. A helper column that only paraphrases the page is not help, it is a
second voice saying the same thing on the screen where the guest is least patient. Same
rule as the §6 "no card" list on mobile.

### 8.1 Screen-id registry (deep-link hashes)

`pd-cart`* · `pd-later` · `pd-removing` · `pd-removed` · `pd-petfee` ·
`pd-petfee-blocked` · `pd-history` · `pd-form` · `pd-form-addon` ·
`pd-processing` · `pd-success` · `pd-error` · modals `m-hold`, `m-remove`,
`m-remove-error`, `m-petfee`, `m-taxinfo` · row popovers `cm-food`, `cm-tax`,
`cm-pet` (* = `data-start`; autonext: processing→success 3.0 s)
...
### 9.0 The three v2 rules, now applied here

A V2 rebuild (since removed) established three business-logic rules. **v1 is the flow that
ships**, so the rules were folded back into it — v1 keeps its math card, season note and
ledger, and gains:

1. **An exemption is a ZERO-COST option.** The exempt row shows a green `.tm-zero`
   "✓ €0.00" plus `.tm-saved` naming what it is worth, and the ledger's exempt line is a
   green `.lg-row.zero`. The old treatment was a **dashed** grey chip — which in this
   prototype means "not charged *yet*" (§7.1 holds). An exemption is not pending; it is
   free, and the row keeps full contrast because an exempt guest is not a demoted one.
2. **Claiming an exemption belongs to the guest.** The page-level "Add guest with
   exemption" row and its "Exemptions" section are gone; every guest row carries a `⋯`
   (`.tm-menu` → sheet on mobile, `.rm-pop` via `rowmenu.js` on desktop) whose contents
   depend on state — taxed guests get "Add an exemption", exempt guests get document /
   change / remove. `t-paid` and `t-empty` carry **no** menu: paid taxes are locked, and a
   region with no exemption types has nothing to offer. A single `.tax-hint` replaces the
   old section. Because the guest arrives preselected, the picker drops its "Exempted
   guest" select and leads with her own outcome ("Ana pays €0.00").
3. **Guests and nights are fixed by the reservation.** The "Edit guests" pen is gone from
   every screen — it promised an edit the reservation does not allow.

### 9.0.1 Guests who have not registered yet (`t-unregistered` / `td-unregistered`)

The tax counts **people, not registrations** — and the data model says so:
`GuestTaxExemption` carries a **required `age`** but an **optional `guest_id`**, so an
entry can exist for someone who is on the booking without being a registered guest. The
real app already renders those rows as `t('unregistered_guest')` → **"Unregistered
guest"** (`ExemptionUser.tsx`), and makes precisely *those* rows the editable ones.

The prototype now shows that state: a `.tm-row.unregistered` with a dashed ghost avatar,
no name, an amber "No details yet" `.tm-tag` — and its **amount unchanged**, because the
reservation already knows how many people there are and which age bracket each falls in.
Its `⋯` offers "Add an exemption" (claimable *before* they register — same person, same
exemption) and "Finish their registration", which links into §2. The `.tax-hint` states
the rule outright: registering changes the **name** on the row, not the amount, unless
their age or an exemption changes.

The list ends with the roster's own **`.add-row` "Add guest"** (→ `t-newguest` /
`td-exemption`) rather than a text link buried in the hint. It is the same gesture on the
same booking, so `.add-row` moved out of `reg.css` into **`shared.css`**: the guest roster
and the tax guest list now render one definition, and the desktop roster keeps only its
size override. Paid taxes get no add-row — the list is a record by then.

Mind the mock-data drift: the taxes flow casts María / Ana / Carlos García while the
registration roster (§2) casts María García / Carlos Ruiz / "Second guest". Same booking,
two different guest lists — worth reconciling before either is used as a reference.
...
| No results | SearchX + "No results for …" + try-a-different-word hint | `h-empty` adds escape actions (improvement): "Vela tips" (`m-vela`) + "Browse all questions" (→ `h-faq`) — never a dead end |
...
- Vela rail is stage-aware: `hd-faq`/`hd-es` — language tip (→ `hd-lang`) +
  conditionality tip; `hd-search` — the two-ages tip; `hd-lang` — minimal (intro
  only, §6 during-overlay rule). No ask box anywhere — Vela has no chat.
...
### 15.0 The door card — the lock face

The one thing on this screen that gets a guest inside is the entry code, and it used to
sit in a **dashed grey box, set smaller than the page title** — quieter than the heading
above it, in the one visual language this prototype reserves for "never charged" (§7.1
holds). The redesign spends its boldness there and nowhere else:

- **The code is the signature.** Each character on its own **key-cap** (`.dc-caps i`) in
  tabular monospace — the way a code is actually read while it is being typed on a keypad.
  No panel behind them: the caps carry the emphasis themselves and the card stays one clean
  surface. On a phone Copy takes its own full-width row rather than fighting the caps for
  one line and losing its tap target.
- **The card stays quiet** so the strip can be loud, and its 3 px edge (`.door::after`)
  carries **state, not identity** — emerald when the door is live, neutral when it is not.
  The two doors are peers on one path and the glyph already says which is which; colouring
  them differently only implied a rank that does not exist.
- **The slide reads armed, not disabled.** The track was grey-on-grey; it is now a tinted
  well with the brand gradient on the *thumb* (the thing you actually grab), and the fill
  behind it is a blue trail that turns emerald once armed. (A version that greened
  *continuously* with the drag was tried and reverted — the threshold flip is the clearer
  signal.)
- **A dedicated unlock green** (`--unlock-1/2/3`, emerald `#0b9e6e → #34d399`), scoped to
  this flow rather than changing the shared `--green-*` tokens: that scale is a mint
  (`#35e5bc`) tuned for quiet "complete" badges, and against white it reads teal. Opening a
  door is the most affirmative moment in the app and deserves a true green.
- Ambient motion (the live dot, the chevron hint) is a hint, never the message:
  `prefers-reduced-motion` stills both and the screen loses nothing.

Cut during the pass: a connector line drawn between the two cards to encode "in order".
The card's own `overflow: hidden` clipped it, and the section label already says it —
the sequence did not need a second voice.
...
| Glass | — | **Minimal** (`html.glass-lite`, baked onto `<html>` — the only version) solidifies the sheet/card/method/group surfaces and drops the sweep, the orbs and the bloom — **the hero photo stays**, because it is content, not chrome |
...
· **Prototype:** removed — §17.0 (`flow-search-v2.html`) is the Find-booking flow
...
**Prototype:** removed — §17.0 (`flow-search-desktop-v2.html`) is the Find-booking flow
...
|---|---|---|---|
| `err-link`* | default `ErrorView` — bad token / auth-init fail / missing deep-link params | `unlink`, blue | primary **"Find my booking"** → `flow-search-v2.html#s2-method`; **Contact your host** (`m-contact-host`); a 2-item "what to check" list (from `not_found_make_sure`) |
...
| Screen | CTA / action |
|---|---|
| `err-link`* | "Find my booking" → `flow-search-v2.html#s2-method`; "Contact your host" → `m-contact-host` |
| `err-rate` | "Try again" → `flow-search-v2.html#s2-method` (countdown is illustrative — static `--p` on `.ec-dial`) |
| `err-crash` | "Reload & try again" → `flow-search-v2.html#s2-method`; "Contact support" → `m-support`; ref "Copy" is a no-op affordance |
| `err-embedded` | "Try again" → `flow-search-v2.html#s2-method` (widget reload) |
...
- `erd-link` / `erd-rate` / `erd-crash`: the variant `.err-art` + copy + actions inside
  the glass card; link/crash CTAs target the **desktop** search (`flow-search-desktop-v2.html#sd2-method`);
...
- `g-empty`: `no_guidebooks_title/subtitle` + an exit (Vela tips / FAQ).
...
- `m-vela` (right sheet): help — jump list (no chat; Vela is tips only).
...
- `gd-html`: `GuidebookHtmlView` — `html_content` in the same doc shell as the PDF, with
  the **"web page · auto-translated"** badge (§21.6). The desktop flow previously had no
  HTML screen at all and its Web card dead-ended on the grid; all three `content_type`s
  now have a desktop reader.
...
`gd-list`* · `gd-guide` · `gd-recos` · `gd-html` · `gd-pdf` · `gd-unavailable` (* = `data-start`)
...
**Glass variants removed** — the redesign settled on flat boxes and chrome-only glass, so the former A/B switch here is gone; the Minimal skin (`html.glass-lite`, via `auth.css`) is baked onto `<html>` as the only version.
...
3. **The account is optional, and the page says so three times.** The reservation link
   alone is enough to check in — the account only buys *next time*. The page used to
   admit this once, in a grey `.auth-alt` text link sitting under a Google button, which
   reads as a third-choice afterthought rather than a right. It is now stated at each
   level of attention a guest actually reads: a green **"✓ Optional"** pill on the title
   (`.auth-optional`), a clause in the sub ("You can also skip it and check in right
   now"), and a real bordered **`.auth-skip`** card replacing the text link — icon,
...
- `a-signin` (`data-start`): create-account email step. Value-prop perks (why an account),
  email field, Google (`m-oauth`), the **opt-out** (below), switch to `a-login`,
...
- `ad-signin` (`data-start`): create-account email + Google + the §23.1 rule-3 opt-out
  (title pill, sub clause, `.auth-skip` card) + switch to login. The opt-out lives entirely
  in the form column: the brand panel makes the case for the account and says nothing about
  skipping it, because the answer belongs beside the thing being answered.
- `ad-pwd`: create-account password step — show/hide eye + strength bars, the desktop twin of
  `a-password`. Desktop reached `ad-code` straight from the email step before this screen
  existed, which skipped the middle of a flow whose own subtitle counts "step 2 of 3".
...
`ad-signin`* · `ad-pwd` · `ad-code` · `ad-login` · `ad-welcome` · `ad-type` · `ad-guests` ·
`ad-who` · `ad-leader` · modals `m-creating`, `m-oauth` (* = `data-start`)
...
**Prototype:** `flow-chat-desktop.html` (loads `chat.css`, `chat-demo.js`)
...
### 33.1 Relationship to §7 payments

One decision reached from two places, and **this section is the only prototype of
it** — payments used to carry its own chooser (`p-deposit`/`pd-deposit`), which was
removed rather than kept in sync with the three backend shapes below. `/deposit` is
a **check-in task** (entered from Home, exits into payments); `dp-edit`/`dpd-edit` is
the same page as an **edit** (entered from the §7 cart's "Edit protection" —
`onEditProtection` → `PATHS.deposits` with `location.state.edit` — and exits back to
it). Both render the identical `.opt-card`; only the header, the CTA label
(`confirm_and_go_back_to_payment`) and the back target differ.
...
`components/*` (`PhaseNav`, `TripMapsButton`, `DaySubTabs`, `DayTimeline`, `PlanEntry`,
...
### 35.8 V3 — the Redesign 2.0 refresh (`flow-travel-guide-v3.html` / `-desktop-v3`, current)

V2 above critiqued the **old** legacy view. The shipped view has since been rebuilt
(tall destination photo bleeding into white, one question per step, description cards
for single-selects) and reads well — but in its own visual language. **V3 keeps the
shipped structure and copy 1:1 and re-dresses it in Redesign 2.0**: flat 1px-bordered
surfaces (no card shadows), Poppins/navy, kit chips + `RadioCardList`-shaped option
cards, `--gradient-hero`-family painted scene instead of the S3 photo (no fetch → no
broken box). Fixture world stays Villa Serena · Valencia · 12–15 March.

- **Kept from the app:** the photo-top intro with the white ramp (`.t3-hero-fade`),
  eyebrow `HOUSING · DATES` + bare-city display title + `intro_duration/intro_pitch`
  lede, "Open my trip guide" / "Skip questions"; four questions in `const.ts` order
  (pace → interests[2–6 + Other] → budget → avoid[optional + Other]) with the shipped
  copy; the four generation phases + progress bar; PhaseNav (Pre-stay only while
  registration is incomplete) / DaySubTabs / time-of-day groups; web_search slots
  (category dot-label, address → maps, duration, cost dots/Free), esim + upselling
  deal cards (badge · stage · price · Book now / Add to cart · Dismiss); `CheckInCard`.
- **Cheap improvements riding along:** the progress segment being answered is alive
  (the app leaves it unlit); the at-a-glance strip renders on the phone (app:
  `hidden lg:block`); `profile_assumptions` become a visible "Tuned for" row with a
  Refine exit back into the questions; the build screen gains an elapsed clock and a
  "you can leave" note; failure keeps the answers and offers three exits; at the
  interests cap remaining chips disable instead of evicting the oldest pick.
- **Screen ids:** mobile `t3-intro`* · `t3-q1..q4` · `t3-build` (autonext → `t3-plan`,
  9200 ms) · `t3-error` · `t3-plan`; desktop `t3d-intro`* · `t3d-q` (all four questions
  on one screen, flat cards, ungated CTA) · `t3d-build` · `t3d-error` · `t3d-plan`
  (reading column + rail: At a glance / route to Maps / Refine / Rebuild).
- **`travel-guide-v3.js` hooks:** `[data-t3-q]` (+ `data-t3-min`, `data-t3-max`) ·
  `[data-t3-continue]` + `[data-t3-goto]` (the gate is the presence of `[data-goto]`,
  never `disabled`) · `[data-t3-other]` · `[data-t3-cap]` · `[data-t3-hint]` ·
  `[data-t3-build]` + `[data-t3-steps]` + `[data-t3-elapsed]` + `[data-t3-bar]` ·
  `[data-t3-dismiss]` · `[data-t3-add]` · `[data-t3-toast]` + `[data-t3-toast-el]` ·
  `[data-t3-live]`. Same `MutationObserver` composition contract as 35.7.
...
`isTemplateInitialized`) · **Prototypes:** `flow-splash-v2.html` (mobile),
`flow-splash-desktop-v2.html` (desktop · SDK · kiosk).

### 36.0 The surface does not change

The design here is V1's, and deliberately: flat `--splash-bg`, the bare logo in a
reserved box, the three pulsing dots, flat text and solid buttons, and the two
**curtains** that close over all of it. Nothing about the surface is redesigned.

What changes about the curtains is *when they run*. Today they animate from **mount** on
a fixed 5 s clock and are shut by ≈4.3 s, while dismissal waits for `load + 5000 ms` —
so they close over a screen that is not ready and hold a dead brand-coloured rectangle
until the timer expires (§36.2). Here they are a **CSS transition fired on data-ready**:
they close over a screen that is already rendered behind them, and the surface is gone
`CURTAIN` (340 ms) later. Same motion, no dead frame. Being a transition rather than a
mount-time animation also means they *cannot* run when there is nothing to hand off to.

Everything else in this section is **behavioural**: the timing contract, the ladder, the
watchdog, the states, the handoff, the accessibility pass. This screen is on for well
under a second, so there is nothing here worth redesigning.

The dots stay because they are honest *by construction* — indeterminate, incapable of
implying a percentage. What they gain is being real elements inside a labelled
`role="status"` region rather than the shipped `box-shadow: 9999px` trick on an
unlabelled `<div>`, which assistive tech cannot see at all.
...
### 36.4 The loader is honest

The loader is V1's three pulsing dots, and it stays that way on purpose. A dot pulse is
**indeterminate by construction**: it cannot imply a percentage, so it cannot imply the
wrong one. A determinate ring would have to guess at a completion the network has not
delivered, and a guess that stalls at 90 % reads as "broken" — worse than saying nothing.

Honesty therefore lives in the *copy*, not the loader: silent below `QUIET`, naming what
is being fetched past it, admitting slowness at `SLOW`, and giving a way out at
`WATCHDOG` (§36.3). There is no fake progress bar, and no percentage anywhere.

What does change is the drawing. Today the dots are a `box-shadow: 9999px` trick on an
unlabelled `<div>` with `alt=""` on the logo — invisible to assistive tech. Here they are
real elements inside the labelled `role="status"` region, and they honour
`prefers-reduced-motion` by settling visible instead of freezing mid-pulse.
...
- `prefers-reduced-motion` is honoured — the dots settle visible and the curtains cut
  rather than sweep (today's 3 s fade-in, 2 s dot pulse and 5 s curtains are
  unconditional).
...
- Confirming the booking opens the shared `m-added` (`CreateDealCompleteModal`) — the
  same modal every other offer uses, carrying the key line that **the QR ticket is issued
  right after payment**. "Review cart & pay" → the shared §7 payment (`lg-pay`,
  `proc-ring`, autonext) or "Keep browsing extras" (the deal sits in the cart like any
  other extra). There is deliberately no bespoke post-add screen: luggage joins the cart
  exactly like an internal offer.
...
`lg-offer`* · `lg-none` · `lg-pay` · `lg-ticket` · `lg-home` · `lg-empty` ·
modals `m-email`, `m-added`, `m-hours`, `m-vela` (* = `data-start`)
...
- Confirming in `m-email` opens the shared `m-added` (`CreateDealCompleteModal`), same
  as any other offer; "Review cart & pay" → `ld-pay` processing ring (autonext).
...
`ld-offer`* · `ld-none` · `ld-pay` · `ld-ticket` · `ld-home` · `ld-empty` ·
modals `m-email`, `m-added`, `m-hours` (* = `data-start`)

---

## §40 VIP airport lounge — the departure-day partner service

**Real components:** none yet — a **new partner feature** (DragonPass-style lounge
network), specced to land the same way the Radical Storage luggage feature did: a
partner offer type (sibling of `LUGGAGE_SERVICE`), an `AUTO` deal paid through the §7
cart, and a partner artifact (the lounge pass) attached once the deal is PAID.
· **Prototypes:** `flow-lounge-mobile.html` (mobile) and `flow-lounge-desktop.html`
(desktop — form left, sticky ledger right, Vela rail; the pass page adds an
ungrouped **"VIP lounge" sidebar row** that exists only while a paid pass does,
mirroring the luggage-storage convention) + `lounge.css` + `lounge-flow.js`.
Lounge photo scenes (`ph-lounge*`) live in `photos.css`. The catalogue/Home
entry points are prototyped in place: a reco card in `home.html` /
`home-desktop.html` "Handpicked for your trip" and an offer card in
`flow-upselling-mobile.html` / `flow-upselling-desktop.html` "For your stay".

### 40.1 Where the lounge is offered (THE placement rule)

The lounge is a **departure-day** service; it never appears as a generic banner.

| Surface | Condition | Treatment |
|---|---|---|
| Extras catalogue | always (when the partner covers the departure airport) | offer card in the "For your stay" grid (`flow-upselling-{mobile,desktop}.html`) |
| Home "Handpicked for your trip" | same condition | reco card with the "Departure day" chip, in **every** Handpicked section — `home.html` renders two (check-in in progress · check-in complete), `home-desktop.html` one |
| Departure-day strip (`vl-list` hero) | inside 48 h of check-out | check-out time · flight · the gap, then the lounge list |
| Airport-transfer confirmation (`vl-nudge`) | transfer confirmed **and** its arrival is ≥ 90 min before boarding **and** no lounge booked yet | one contextual card ("your car reaches AGP around 18:15") opening the nudge sheet; **shown once** |
| Home (`vl-home`) | a paid pass exists | collapsed pass card in its own section, like the other paid partner bookings |

### 40.2 Airport resolution

The departure airport is **auto-detected** (transfer booking / flight details) and always
changeable (`m-airport` sheet, radio rows, detected row tagged). Three shapes:

| Airport state | Screen | Rule |
|---|---|---|
| lounges exist | `vl-list` | cards render; filter chips (All · After/Before security · terminal) — zero matches shows the in-place no-match block, never a blank |
| no partner lounges | `vl-none` | named empty state + **notify-me watch** + "Book an airport transfer instead"; never a dead end |
| sold out | card stays in the list | muted card, "Sold out today" status, **Notify me if space opens** — availability is stated, not hidden |

### 40.3 Lounge card contract

Every card carries: status pill (**Open now / Opens HH:MM / Sold out today** — partner
availability, never inferred), rating + review count, terminal + before/after security,
amenity chips, **€/person with the max stay beside it**, and today's opening hours.
Selection is radio-style; the docked CTA stays disabled ("Choose a lounge to continue" —
the `findBookingGap` pattern) until one is chosen, then names it ("Book Sala VIP Cristal").

### 40.4 Booking form (`vl-book`)

- **Date is fixed** to the check-out date and says so — the lounge is a departure-day
  service, not a date picker.
- **Entry time** is a slot radio group (16:00 / 17:00 / 18:00 relative to boarding; the
  transfer-aligned slot is annotated "Right after your transfer arrives"). A slot means an
  **entry window** (slot → slot + 1 h) and the **max stay (3 h) counts from entry** — the
  window note under the slots states both.
- **Guests** default to the check-in party size, clamped 1–6; the per-person line and
  total reprice live.
- The lane chip is **⚡ Instant**; the free-cancellation note (24 h before entry, full
  refund) and the partner trust line ("Provided by DragonPass · Chekin partner network")
  sit above the CTA.

### 40.5 Money model (same as every extra)

`vl-added` says exactly what happened: spots **held with the request, confirmed the
moment you pay** — adding never charges. "Review cart & pay €64.00" runs the shared §7
payment; the **QR pass is issued only after the deal is PAID**. The pass (`vl-pass`) is
the shared partner-ticket surface: one QR for the whole party, a fallback **access code**
(copyable), entry window ↔ max stay, party, location, included amenities — and the
cancellation deadline printed **as a date** ("Cancel free until Thu 21 Aug, 18:00").
Cancelling (`vl-cancelled`) names the amount, the card and the 3–5-day refund window,
and offers rebooking in one tap.

### 40.6 Screen-id registry (deep-link hashes)

Mobile: `vl-list`* · `vl-book` · `vl-added` · `vl-pay` · `vl-pass` · `vl-nudge` ·
`vl-home` · `vl-none` · `vl-cancelled`  (* = `data-start`; autonext:
`vl-pay`→`vl-pass` 2.1 s) · modals `m-airport` · `m-lounge` · `m-nudge` · `m-vela`.
Desktop: `vd-list`* · `vd-added` · `vd-pay` · `vd-pass` · `vd-home` · `vd-none` ·
`vd-cancelled`  (autonext: `vd-pay`→`vd-pass` 2.1 s; no separate booking step —
slots and the counter live on `vd-list` beside the ledger; the nudge stays a
mobile scenario) · modals `m-airport` · `m-lounge`.
...
8. **Shared chrome is a component, never a copy.** `chrome.js` owns the sidebar, status
   bar, top bar and **page head**; a view supplies only what is genuinely its own, as
   attributes on an empty element:
   ...
   `{…}` marks the highlighted words — a view never hand-writes `<span class="hl">`, so
   the accent cannot land on the wrong span. `data-accent="ok|bad"` recolours it for
   outcome screens (captured / passed vs. try again) via `.hl.ok` / `.hl.bad`, which is
   why no page-title carries an inline colour. Richer content — an intro with a `<b>` or
   an inline "Edit" button — is written as **slot content** inside the element and
   re-appended after the generated parts, exactly as the top bar does with its trailing
